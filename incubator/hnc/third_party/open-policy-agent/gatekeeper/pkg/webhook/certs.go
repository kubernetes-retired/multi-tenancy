...
// webhooks doesn't get clobbered
...
func (r *ReconcileWH) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	if secret.GetDeletionTimestamp().IsZero() {
		...
		// Ensure certs on CRDs.
		if result, err := r.ensureCRDCerts(artifacts.CertPEM); err != nil {
			return result, err
		}
	}
...
func (r *ReconcileWH) ensureCRDCerts(certPem []byte) (reconcile.Result, error) {
