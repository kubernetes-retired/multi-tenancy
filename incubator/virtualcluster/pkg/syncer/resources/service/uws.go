...
func (c *controller) processNextWorkItem() bool {
	...
	utilruntime.HandleError(fmt.Errorf("error processing service %v (will retry): %v", req.Key, err))
	if c.queue.NumRequeues(req) >= constants.MaxUwsRetryAttempts {
		klog.Warningf(" Service uws request is dropped due to reaching max retry limit: %v", req)
		c.queue.Forget(obj)
		return true
	}
