...
var b = &benchmark.Benchmark{
	...
	Run: func(options types.RunOptions) error {
		...
		role, err := options.Tenant1Client.RbacV1().Roles(options.TenantNamespace).Create(context.TODO(), role, metav1.CreateOptions{})
		if err != nil {
			return err
		}
		...
		_, err = options.Tenant1Client.RbacV1().RoleBindings(options.TenantNamespace).Create(context.TODO(), roleBinding, metav1.CreateOptions{DryRun: []string{metav1.DryRunAll}})
		if err != nil {
			return err
		}
	...
	PostRun: func(options types.RunOptions) error {
		err := options.Tenant1Client.RbacV1().Roles(options.TenantNamespace).Delete(context.TODO(), "role-sample", metav1.DeleteOptions{})
		if err != nil {
			return err
		}
