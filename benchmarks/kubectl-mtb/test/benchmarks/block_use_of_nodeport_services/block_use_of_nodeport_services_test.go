...
func testPreRunWithRole(t *testing.T) (preRun bool, run bool) {
	...
	createdRole, err := testClient.CreateRole("service-role", policies)
	if err != nil {
		g.Expect(err).NotTo(gomega.HaveOccurred())
	}

	_, err = testClient.CreateRoleBinding("service-role-binding", createdRole)
	if err != nil {
		g.Expect(err).NotTo(gomega.HaveOccurred())
	}
...
func testRunWithPolicy(t *testing.T) (preRun bool, run bool) {
	paths := []string{unittestutils.DisallowNodePortServices}
