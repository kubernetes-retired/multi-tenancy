...
func testPreRunWithRole(t *testing.T) (preRun bool, run bool) {
	...
	createdRole, err := testClient.CreateRole("pod-role-4", policies)
	if err != nil {
		fmt.Println(err.Error())
	}

	_, err = testClient.CreateRoleBinding("pod-role-binding-4", createdRole)
	if err != nil {
		fmt.Println(err.Error())
	}
...
func testRunWithPolicy(t *testing.T) (preRun bool, run bool) {
	paths := []string{unittestutils.DisallowHostIPC}
